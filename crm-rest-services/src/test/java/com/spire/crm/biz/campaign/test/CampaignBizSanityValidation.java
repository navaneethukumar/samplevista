...
import com.spire.common.CampaignCandidatePojo;
...
public class CampaignBizSanityValidation {
	...
	public Node setUpFeedBackNode(String parentNodeId, int waitTime) {
		Node node = new Node();
		node.setId(IdUtils.generateUUID());
		node.setName("Read email");
		node.setDescription("Read email");
	...
	public Node setUpSendEmailNode(String parentNodeId, String decisionPath,
		...
		node.setDecisionPath(decisionPath);
		node.setWaitTime(0);
		node.setProperties("{\"templateId\":\"6002:6031:1c313f300b3f4c2d8ac189af6513bba5\",\"templateName\":\"SpireAutomationEmail\"}");
		return node;
	...
	public Campaign setUpCampaign(Boolean isPublished) {
		...
		campaign.setFlowId("179740ba-3b51-48f8-beba-ebaa8f51b124");
	...
	private void compareNodes(Node inputnode, Node outputnode) {
		...
		Assert.assertEquals(inputnode.getDecisionPath(),
				outputnode.getDecisionPath(),
	...
	public Campaign createCampaignWithAllNodes(Boolean isPublished,
		...
		// Stage change Node
		Node sendEmailNode01 = setUpSendEmailNode(null, "0", null);
		...
		if (createResponse.getStatus() == 200) {
			...
			Logging.log("Response after create campaign is >>> "
	...
	public void executeCampaignEvents() {
		 ...
		 * try {
		 * 
		 * String cron_expression = config.getconfig("crm-biz",
		 * "cron.expression");
		 * 
		 * waitTime = (int) cron_expression.charAt(4) - 48;
		 * 
		 * } catch (Exception e) { waitTime = 2; // taking default value when
		 * config is down }
		 */
	...
	public void verifyOneDecisionCampaign(TestData data,
		...
		Node node1SendEmail = setUpSendEmailNode(null, "0", "templateID");
	...
	public Campaign createExecutionCampaign(TestData data,
		...
		Node node1SendEmail = setUpSendEmailNode(null, "0",
	...
	public void verifySentActivity(TestData data, SpireTestObject testObject) {
		CampaignBizServiceConsumer campaignBizServiceConsumer = new CampaignBizServiceConsumer();
		List<Node> nodeLIst = new ArrayList<Node>();
		Node sendEmail = setUpSendEmailNode(null, "0", null);
	...
	private void editCampaignWithAllNodes(Campaign editCampaign) {
		editCampaign.setFlowId("179740ba-3b51-48f8-beba-ebaa8f51b124");
	...
	public void addCampToCandidate(TestData data, SpireTestObject testObject) {
		String campaignId = null, candidateID = null;
		Campaign campaign = null;
		CampaignCandidatePojo campaignCandidatePojo = new CampaignCandidatePojo();
		CampaignLeadVO campaignLeadVO = new CampaignLeadVO();
		CampaignBizServiceConsumer campaignBizServiceConsumer = new CampaignBizServiceConsumer();
		if (campaignId == null) {
			campaign = createCampaignWithAllNodes(true, "Lead");
	...
	public void verifyBatchExecution(TestData data, SpireTestObject testObject) {
		String campaignId = null, candidateID = null;
		CampaignLeadVO campaignLeadVO = new CampaignLeadVO();
		CampaignBizServiceConsumer campaignBizServiceConsumer = new CampaignBizServiceConsumer();
		campaignId = createCampaignWithAllNodes(true, "Lead").getId();
		candidateID = ProfileHelper.createProfile();
		List<String> campaignIds = new ArrayList<String>();
		campaignIds.add(campaignId);
		campaignLeadVO.setCampaignIds(campaignIds);
